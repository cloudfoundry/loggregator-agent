...
var _ = Describe("Prometheus Registry", func() {
	...
	It("registers the gauge when it's first gotten", func() {
		...
		Expect(gauge.Desc().String()).To(Equal(`Desc{fqName: "origin_metric_name", help: "vm metric", constLabels: {unit="unit"}, variableLabels: []}`))

		var registered prometheus.Gauge
		Eventually(registerer.gauges).Should(Receive(&registered))

		Expect(registered.Desc().String()).To(Equal(`Desc{fqName: "origin_metric_name", help: "vm metric", constLabels: {unit="unit"}, variableLabels: []}`))
	})
	...
	It("gauges with different tags are different gauges", func() {
		...
		Expect(gauge.Desc().String()).To(Equal(`Desc{fqName: "origin_metric_name", help: "vm metric", constLabels: {foo="bar",unit="unit"}, variableLabels: []}`))
